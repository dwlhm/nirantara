                        ...
                        .pointerInput(card.id, corner) {
                            detectDragGestures(
                                ...
                                onDrag = { change, dragAmount ->
                                    ...
                                    val newW = card.widthCells * columnWidthPx + deltaW
                                    val newH = card.heightCells * rowHeightPx + deltaH

                                    // For simplicity in this example, snapping by threshold
                                    val newCellsW = (newW / columnWidthPx).roundToInt().coerceIn(1, 4)
                                    val newCellsH = (newH / rowHeightPx).roundToInt().coerceAtLeast(1)

                                    if (newCellsW != card.widthCells || newCellsH != card.heightCells) {
                                        onSizeChange(newCellsW, newCellsH)
                                        // Reset drag to prevent overshoot
                                        dragX = 0f
                                        dragY = 0f
                                    } else {
                                        dragX += dx
